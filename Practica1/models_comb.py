...
for file in os.listdir('data/comb'):
    ...
    for i, model in enumerate(models):
        ...
        print(f'Top 3 accuracy model {i+1}: {round(top_k_accuracy_score(y_test, prediction, k=3)*100,2)}%')
        # print accuracy, precision, f1-score
        langs = ['en', 'ca', 'es', 'de', 'fr', 'pl', 'pt', 'ru', 'it', 'sv']
        report = classification_report(y_test, model.predict(X_test_pca), target_names=langs, output_dict=True)
        print("Accuracy: ", round(report['accuracy']*100, 2), "Precision: ", round(report['macro avg']['precision']*100,2), "F1-score: ", round(report['macro avg']['f1-score']*100,2))